...
data=pd.read_csv('../data/healthcare-dataset-stroke-data.csv', dtype={col: np.float32 for col in ['bmi']})
...
#print(data_log)
rows=data_log.loc[data_log['bmi'].isnull()] #column with nan values of bmi
...
index_list =rows.index #save the index of the rows 



regr = KNeighborsRegressor(n_neighbors=3).fit(X_train, Y_train)
...
bmi_pred=bmi_pred.set_index([pd.Index(index_list)]) #put the index at the new dataframe 
